...
build:
	...
	go build -v -ldflags '-X "github.com/andig/ingress/cmd.version=${VERSION}" -X "github.com/andig/ingress/cmd.commit=${SHA}" -X "github.com/andig/ingress/cmd.date=${BUILD_DATE}"' github.com/andig/ingress

run:
	go run -race github.com/andig/ingress
